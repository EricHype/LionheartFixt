**0.5.0 was packaged and never published. Do not use it if you have a copy: it
crashes fatally on entering the vault, twice over.**

Both were art referenced without being checked against the game's archive -- a chest
placed with a model path that does not exist, and then the same chest set to an
animation that model does not have, found immediately after fixing the first. Both are
now gates in `tools/validate.py`, written against the real crashes before they were
fixed. A sweep of every entity this mod adds found no other instance of either.

Also fixed: a guard who attacked after you had passed his Speech check. The reply stood
down his *generator* rather than the man, and the man carries his own proximity trigger.

**Read this first: this is not a signed-off build.**

The final job has been walked end to end. The vault has been walked as far as its
entrance, which is how both crashes were found; nobody has yet completed it. The
caught-in-the-act branch is **unplayed**. Everything here is honest about which is
